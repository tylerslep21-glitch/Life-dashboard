...
router.get('/', async (req, res) => {
  await sweepExpired(req.db);
  const { rows } = await req.db.query('SELECT * FROM todos ORDER BY position ASC, id ASC');
  res.json(rows);
});
...
router.post('/', async (req, res) => {
  ...
  const { rows: maxRows } = await req.db.query('SELECT COALESCE(MAX(position), -1) AS max_pos FROM todos');
...
router.delete('/:id', async (req, res) => {
  await req.db.query('DELETE FROM todos WHERE id = $1', [req.params.id]);
  res.status(204).end();
});
