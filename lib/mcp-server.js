...
function buildMcpServer() {
  ...
  server.registerTool(
    'post_robinhood_snapshot',
    {
      ...
      description:
        'Records a Robinhood account value snapshot (current total + optional daily history) for Overview (Life Dashboard). Each call inserts a new row - call once per account per sync.',
      inputSchema: {
    ...
    async ({ account_label, total_value, history }) => {
      ...
      const { rows } = await pool.query(
        ...
        [account_label, total_value, encryptJSON(safeHistory), userId]
      );
      return {
  ...
  server.registerTool(
    'post_agent_status',
    {
      ...
      description:
        "Upserts a scheduled agent's status card on Overview's Agent Tracker (keyed by agent_name - re-posting the same name overwrites its row, this is a status snapshot not a log).",
      inputSchema: {
        agent_name: z.string(),
  ...
  server.registerTool(
    'add_exams',
    {
      title: 'Add exams/big-project dates',
      description:
        "Adds one or more entries to Overview's Exams / Big Projects widget for tslep - exams, quizzes, project deadlines, presentations, anything date-based worth tracking there. Each call is a plain insert (no dedup), so don't re-call with the same dates already added.",
      inputSchema: {
        exams: z.array(z.object({
          name: z.string().describe('e.g. "Midterm Exam", "Quiz 3", "Pinnacle Project Part 2 (individual) due"'),
          event_date: z.string().describe('ISO date, YYYY-MM-DD'),
          course: z.string().optional().describe('e.g. "BUS-J 375" - shown alongside the name on the dashboard'),
        })).min(1),
      },
    },
    async ({ exams }) => {
      ...
      for (const e of exams) {
        const { rows } = await pool.query(
          `INSERT INTO exams (name, event_date, course, user_id) VALUES ($1, $2, $3, $4)
           RETURNING id, name, event_date, course`,
          [e.name, e.event_date, e.course || null, userId]
        );
        inserted.push(rows[0]);
      }
      return {
        content: [{ type: 'text', text: `Added ${inserted.length} exam(s): ${JSON.stringify(inserted)}` }],
      };
    }
  );
